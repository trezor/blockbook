...
func (d *RocksDB) GetAddrDescTransactions(addrDesc bchain.AddressDescriptor, lower uint32, higher uint32, assetsBitMask bchain.AssetsMask, fn GetTransactionsCallback) (err error) {
	...
	for it.Seek(startKey); it.Valid(); it.Next() {
		...
		for len(val) > txidUnpackedLen+1 {
			...
			if (assetsBitMaskUint & maskUint) == maskUint {
		...
		for i, input := range tx.Vin {
			...
			if d.chainParser.IsAddrDescIndexable(spentOutput.AddrDesc) {
				...
				if tai.AssetInfo.AssetGuid > 0 {
					...
					err := d.ConnectAllocationInput(block.Height, balanceAsset, isAssetTx, tx.Version, btxID, &tai.AssetInfo, assets, txAssets)
					if err != nil {
			...
			if d.chainParser.IsAddrDescIndexable(t.AddrDesc) {
				...
				if balance != nil {
					...
					if t.AssetInfo.AssetGuid > 0 {
						...
						err := d.DisconnectAllocationOutput(balance.AssetBalances, isActivate, isAssetTx, txa.Version, btxID, assets, &t.AssetInfo, assetFoundInTx)
						if err != nil {
