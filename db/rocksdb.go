...
const addrContractsCacheMinSize = 5_000 // limit for caching address contracts in memory to speed up indexing
const addrContractsSkipSize = 10_000
...
type RocksDB struct {
	...
	addrContractsCacheState addrContractsCacheState
}
...
func NewRocksDB(path string, cacheSize, maxOpenFiles int, parser bchain.BlockChainParser, metrics *common.Metrics, extendedIndex bool) (d *RocksDB, err error) {
	...
	r := &RocksDB{
		...
		connectBlockMux: sync.Mutex{},
		addrContractsCacheState: addrContractsCacheState{
			cache:           make(map[string]*unpackedAddrContracts),
			cacheMeta:       make(map[string]*addrContractsCacheMeta),
