...
PORTABLE = 0
ARGS ?=
# Forward BB_BUILD_ENV, BB_*_RPC_URL_*, BB_RPC_*, and BB_TEST_API_* overrides into Docker for build/test tooling.
BB_RPC_ENV := $(shell env | awk -F= '/^BB_BUILD_ENV$$|^BB_(DEV|PROD)_RPC_URL_(HTTP|WS)_|^BB_RPC_(BIND_HOST|ALLOW_IP)_|^BB_TEST_API_URL_(HTTP|WS)_/ {print "-e " $$1}')
