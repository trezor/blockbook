...
func (w *Worker) GetTransactionFromBchainTx(bchainTx *bchain.Tx, height int, spendingTxs bool, specificJSON bool) (*Tx, error) {
				...
				if tas == nil {
					...
					if len(otx.Vout) > int(vin.Vout) {
						...
						if vout.AssetInfo != nil {
							vin.AssetInfo = &bchain.AssetInfo{AssetGuid: vout.AssetInfo.AssetGuid, new(big.Int).Set(vout.AssetInfo.ValueSat)}
						}
					}
				} else {
					if len(tas.Outputs) > int(vin.Vout) {
						...
						if output.AssetInfo != nil {
							vin.AssetInfo = &bchain.AssetInfo{AssetGuid: output.AssetInfo.AssetGuid, new(big.Int).Set(output.AssetInfo.ValueSat)}
						}
					}
				}
				if vin.ValueSat != nil {
	...
	for i := range bchainTx.Vout {
		...
		if bchainVout.AssetInfo != nil {
			...
			vout.AssetInfo = &bchain.AssetInfo{AssetGuid: bchainVout.AssetInfo.AssetGuid, new(big.Int).Set(bchainVout.AssetInfo.ValueSat)}
					...
					for i := range bchainTx.Vout {
						...
						if err == nil && bytes.Equal(addrDesc, vad) {
							...
							if !e {
								...
								if vout.AssetInfo != nil {
									utxoTmp.AssetInfo = &bchain.AssetInfo{AssetGuid: vout.AssetInfo.AssetGuid, new(big.Int).Set(vout.AssetInfo.ValueSat)}
								}
				...
				if !e {
					...
					if !e {
						...
						if utxo.AssetInfo != nil {
							utxoTmp.AssetInfo = &bchain.AssetInfo{AssetGuid: utxo.AssetInfo.AssetGuid, new(big.Int).Set(utxo.AssetInfo.ValueSat)}
						}
						utxos = append(utxos, utxoTmp)
					}
				}
