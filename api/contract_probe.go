...
// resolveContractInfo returns the pre-pass verdict for cd when the batch covered it, else
// reads this one contract from the chain, collapsing concurrent probes of the same address.
// The pair is a verdict: info==nil with err==nil means the chain conclusively reports no
// token at the address, so the caller may cache it.
...
// copyContractInfo hands every caller its own struct: both the probe cache and the batch
// pre-pass share one result across callers that go on to mutate and store it.
...
// unknownContractInfo is how an unresolved contract is surfaced: no name or symbol and the
// coin's decimals. Callers pair it with validContract=false, which keeps the contract out of
// balance reads.
...
// contractNeedsChainRefresh reports whether a stored record still has to be read from the
// chain: an unhandled standard (sync skips the metadata fetch for contracts it indexes from
// internal transactions) or a name/symbol that parsed as leading zeroes.
