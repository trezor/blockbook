...
func (p *PivXParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
	...
	for i, in := range t.TxIn {
		...
		// remove txid from OP_ZEROCOINSPEND inputs
		if(isZeroCoinSpendScript(in.SignatureScript)) {
			txid = ""
		}
...
func (p *PivXParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	...
	for i := range tx.Vin {
		...
		signatureScript, _ := hex.DecodeString(scriptSigHex)

		if(isZeroCoinSpendScript(signatureScript)) {
			vin.Txid = ""
		}
	}
