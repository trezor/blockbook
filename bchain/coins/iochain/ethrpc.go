...
func NewIochainRPC(config json.RawMessage, pushHandler func(bchain.NotificationType)) (bchain.BlockChain, error) {
	var err error
	var c Configuration
...
func (b *EthereumRPC) GetTransaction(txid string) (*bchain.Tx, error) {
	...
	if tx.BlockNumber == "" {
		...
		// again, try to pull the private transaction receipt but fail silent if this does not work
		b.RPC.CallContext(ctx, &receipt, "priv_getTransactionReceipt", hash)
...
func (b *EthereumRPC) SendRawTransactionPublic(hex string) (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.Timeout)
	defer cancel()
	var raw json.RawMessage
...
// SendRawTransactionPrivate sends raw private transaction
func (b *EthereumRPC) SendRawTransactionPrivate(hex string) (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), b.Timeout)
	defer cancel()
	var raw json.RawMessage
	err := b.RPC.CallContext(ctx, &raw, "eea_sendRawTransaction", hex)
	if err != nil {
		return "", err
	} else if len(raw) == 0 {
		return "", errors.New("SendRawTransaction: failed")
