...
func TestParseBlock(t *testing.T) {
	p := NewcXAUParser(GetChainParams("main"), &btc.Configuration{})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("cXAUParser.ParseTxFromJson() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("cXAUParser.ParseTxFromJson() = %+v, want %+v", got, tt.want)
			}
		})
	}
}
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				height:    1028160,
				blockTime: 1551246710,
				parser:    NewcXAUParser(GetChainParams("main"), &btc.Configuration{}),
			},
			want:    testTxPacked1,
			wantErr: false,
		},
