...
func NewEthereumRPC(config json.RawMessage, pushHandler func(bchain.NotificationType)) (bchain.BlockChain, error) {
	...
	go func() {
		for {
			...
			s.PushHandler(bchain.NotificationNewTx)
		}
	}()
...
func openRPC(url string) (*EthereumRPCClient, *EthereumClient, error) {
	r, err := rpc.Dial(url)
	if err != nil {
		return nil, nil, err
	}

	rc := &EthereumRPCClient{
		Client: r,
	}
...
func (b *EthereumRPC) Initialize() error {
	...
	go func() {
		for {
			...
			b.UpdateBestHeader(&EthereumHeader{Header: h})
			// notify blockbook
