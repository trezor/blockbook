...
func (p *BaseParser) UnpackVarBytes(buf []byte) ([]byte, int) {
	txvalue, l := p.UnpackVarint(buf)
	bufValue := append([]byte(nil), buf[l:l+txvalue]...)
...
func (p *BaseParser) CompressAmount(n uint64) uint64 {
    if n == 0 {
		return 0
	}
    var e int = 0;
...
func (p *BaseParser) DecompressAmount(x uint64) uint64 {
