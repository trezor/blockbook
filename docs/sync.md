...
- **The cached tip also spares the header RPCs of a steady-state EVM cycle.** `resyncIndex` reads one `EthereumTypeGetBestTip` snapshot (hash, parent hash, height from the same header). When the tip is exactly one block above the local best *and* its parent hash is the local best hash, the pushed header extends the indexed chain, which is all the `GetBlockHash(localBestHeight)` fork check would re-confirm, so it is skipped and the tip hash is used as the start hash; `getBlockChain` consults the cached height before the end-of-chain probe instead of sending an `eth_getBlockByNumber` that returns `null`. A one-block cycle therefore costs only the block fetch (body, `eth_getLogs`, trace). Any other shape — a gap above one, a parent mismatch, a tip that regressed — takes the original path with the fork check intact. The match proves the local chain is the parent of the node's tip, not that the tip stays canonical: a same-height sibling that later wins is indexed for one cycle and swapped out by the next cycle's fork check, as it was before the shortcut. Bitcoin-type chains keep the original sequence: their `GetBestBlockHeight` is a real RPC and `GetBlock` leaves `Next` empty for every block.
- **The EVM parallel and bulk producers enqueue heights only; the writer verifies parent linkage.** `ParallelConnectBlocks` and `BulkConnectBlocks` used to resolve `GetBlockHash(h)` for every height on the single producer goroutine before a worker fetched the same block by hash. On EVM `GetBlock("", h)` is one `eth_getBlockByNumber` that already carries the body, so the producers send bare heights and the header RPC is gone (`parallelFetchByHeight`). Without an expected hash a missing block is no longer evidence of a reorg (`shouldRestartSyncOnMissingBlock` short-circuits, the stall cap still bites), so reorg detection moves to `writeBlockWorker`, the only goroutine that sees blocks in height order: it seeds the previous hash from the local best block and rejects a block whose `Prev` does not match (`unlinkedParent`, counted as `index_reorg_events{type="fork"}`), yielding `errResync` before the unlinked block is connected. The next `resyncIndex` then fails the `linked` shortcut, its `GetBlockHash(localBestHeight)` fork check finds the orphan and `handleFork` unwinds it. This also covers a load-balanced pool where one worker read height *h* from a lagging node and a sibling read *h+1* from a current one. Bitcoin-type chains keep one `getblockhash` per height because their `GetBlock("")` would issue it internally anyway, and their writer skips the linkage check: a hash-pinned round already follows the node's canonical hash chain.
...
| `RecheckThreshold`    | 10              | `getBlockWorker` mid-queue                                                      | Retries before calling `shouldRestartSyncOnMissingBlock` (a no-op for hashless EVM fetches; `MaxStallDuration` bounds those) |
