...
- **The cached tip also spares two RPCs per steady-state EVM cycle.** `resyncIndex` takes the start hash straight from the cached header when the block to connect *is* the tip (one block behind), and `getBlockChain` consults the cached height before the end-of-chain probe instead of sending an `eth_getBlockByNumber` that returns `null`. The `GetBlockHash(localBestHeight)` fork check is untouched, so a one-block cycle costs one header lookup plus the block fetch (body, `eth_getLogs`, trace). Bitcoin-type chains keep the original sequence: their `GetBestBlockHeight` is a real RPC and `GetBlock` leaves `Next` empty for every block.
