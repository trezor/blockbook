...
- **The cached tip also spares the header RPCs of a steady-state EVM cycle.** `resyncIndex` reads one `EthereumTypeGetBestTip` snapshot (hash, parent hash, height from the same header). When the tip is exactly one block above the local best *and* its parent hash is the local best hash, the pushed header has already proven the indexed chain canonical, so the `GetBlockHash(localBestHeight)` fork check is skipped and the tip hash is used as the start hash; `getBlockChain` consults the cached height before the end-of-chain probe instead of sending an `eth_getBlockByNumber` that returns `null`. A one-block cycle therefore costs only the block fetch (body, `eth_getLogs`, trace). Any other shape — a gap above one, a parent mismatch, a tip that regressed — takes the original path with the fork check intact. Bitcoin-type chains keep the original sequence: their `GetBestBlockHeight` is a real RPC and `GetBlock` leaves `Next` empty for every block.
