...
-   `<network>_WS_MESSAGE_RATE_LIMIT` - Maximum number of messages a single WebSocket connection may send within `<network>_WS_MESSAGE_RATE_WINDOW` before the connection is closed and (when blockable) its client key is added to the IP blocklist. Accepts a non-negative integer; default `5000`, `0` disables the per-connection message rate limit entirely. The default of 5000 messages / 10 minutes is above the maximum burst produced by a Trezor Suite client, so it only trips clearly abusive traffic.
...
-   `<network>_WS_MAX_CONNECTIONS_PER_IP` - Maximum number of concurrent WebSocket connections allowed from a single client IP (aggregated to its `/64` for IPv6). Accepts a non-negative integer; default `128`, `0` disables the cap entirely (unlimited connections per IP). IPv6 addresses are grouped by `/64` prefix before counting, so an abuser cannot exhaust the limit by rotating /64-local addresses.
