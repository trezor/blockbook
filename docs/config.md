    ...
    * `block_chain` – Configuration of BlockChain type that ensures communication with back-end service. All options
        ...
        * `additional_params` – Object of coin-specific params.
          ...
          * Ethereum mempool timeout configuration:
            ...
            * `mempoolTxTimeout` – Optional Blockbook-side EVM mempool retention as a Go duration string such as `"10m"`; `"0s"` preserves the legacy zero-retention setting, but note it inverts the ordering below and so fails startup on a relay-enabled coin. With an alternative send transaction provider enabled it defaults to `alternativePendingTxWindow` plus a **30 minute** margin instead of the legacy hour-based value, and must stay strictly longer than the alternative cache retention — Blockbook refuses to start otherwise, because the mempool sweep would drop a private transaction's address index while the provider cache still serves it as pending.
            * `alternativePendingTxWindow` – How long a transaction broadcast through the alternative (private relay) provider is treated as still pending, as a positive Go duration string. Defaults to **3 hours** — the window in which a private relay's transaction can still be built into a block, and (since the Blinklabs alignment) also the window over which the relay answers `eth_getTransactionByHash` and counts the transaction at the pending `eth_getTransactionCount` tag. It drives the alternative cache retention and, through it, `mempoolTxTimeout` above.
            * `alternativeMempoolTxTimeout` – Optional alternative-provider transaction cache retention as a positive Go duration string. Defaults to `alternativePendingTxWindow`; override only to hold private transactions for a *different* period than the one in which they can still land.
            * `alternativeMissingTxTimeout` – How long a cached private transaction may stay missing from the relay's `eth_getTransactionByHash` (consecutive null answers) before Blockbook treats it as dropped or cancelled and evicts it, as a positive Go duration string. Defaults to **2 minutes** — the relay answers from one consistent store, so a null means the transaction is gone, and the tolerance only absorbs a transient relay fluke. Set it at or above `alternativePendingTxWindow` for a relay that does not surface accepted transactions over its whole window — that restores eviction by cache timeout only.
