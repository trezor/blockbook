    ...
    * `block_chain` – Configuration of BlockChain type that ensures communication with back-end service. All options
        ...
        * `additional_params` – Object of coin-specific params.
          ...
          * Ethereum mempool timeout configuration:
            ...
            * `mempoolTxTimeout` – Optional Blockbook-side EVM mempool retention as a Go duration string such as `"10m"`; `"0s"` preserves the legacy zero-retention setting. If omitted and an alternative send transaction provider is enabled, Blockbook uses `alternativePendingTxWindow` plus a **30 minute** margin instead of the legacy hour-based value. It must stay strictly longer than the alternative cache retention — Blockbook refuses to start otherwise, because the wrapped mempool's timeout sweep would drop a private transaction's address index while the provider cache still serves it as pending.
            * `alternativePendingTxWindow` – How long a transaction broadcast through the alternative (private relay) provider is treated as still pending, as a positive Go duration string. Defaults to **3 hours**, the window in which a private relay's transaction can still be built into a block. It is not the same as how long the relay keeps *surfacing* the transaction through `eth_getTransactionByHash`: the relay dropping it from that answer is not proof it is gone. This setting drives both the alternative cache retention and, through it, the Blockbook mempool retention above.
            * `alternativeMempoolTxTimeout` – Optional alternative-provider transaction cache retention as a positive Go duration string. Defaults to `alternativePendingTxWindow`; set it only to hold private transactions for a *different* period than the one in which they can still land.
