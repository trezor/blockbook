    ...
    * `block_chain` – Configuration of BlockChain type that ensures communication with back-end service. All options
        ...
        * `additional_params` – Object of coin-specific params.
          ...
          * Ethereum mempool timeout configuration:
            ...
            * `mempoolTxTimeout` – Optional Blockbook-side EVM mempool retention as a Go duration string such as `"10m"`; `"0s"` preserves the legacy zero-retention setting. With an alternative send transaction provider enabled it defaults to `alternativePendingTxWindow` plus a **30 minute** margin instead of the legacy hour-based value, and must stay strictly longer than the alternative cache retention — Blockbook refuses to start otherwise, because the mempool sweep would drop a private transaction's address index while the provider cache still serves it as pending.
